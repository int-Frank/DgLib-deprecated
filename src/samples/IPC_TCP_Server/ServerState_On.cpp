...
namespace IPC
{
  static void Run_Listen(ServerState_On * a_pApp, SocketData a_listenSocket)
  {
  ...
  ServerState_On::ServerState_On(TCP_Server * a_pApp, SocketData const & a_listenSocketData)
  ...
  void ServerState_On::RegisterClient(SocketData const & a_sd)
  {
    m_clientHandler.AddClient(a_sd);
  }

  void ServerState_On::DeregisterClient(SocketData const & a_sd)
  {
  ...
  std::vector<SocketData> ServerState_On::GetClientList()
  {
