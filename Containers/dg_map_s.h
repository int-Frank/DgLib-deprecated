...
namespace Dg
{
  ...
  void map<U, T>::init(const map& other)
  {
	  ...
	  {
      new (&m_data[i].key) U(other.m_data[i].key);
      new (&m_data[i].item) T(other.m_data[i].item);
	  }
  ...
  bool map<U, T>::insert(const T& val, U key)
  {
    ...
    new (&m_data[index].key) U(key);
    new (&m_data[index].item) T(val);
