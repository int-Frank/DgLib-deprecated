...
namespace Dg
{
  ...
  public:
	  ...
	  //Accessors
	  iterator			  begin()		  const {return iterator(root_container.next);}
    iterator			  end()		    const {return iterator(const_cast<data_container*>(&end_container)); }
	  const_iterator	cbegin()	  const {return const_iterator(root_container.next);}
    const_iterator	cend()		  const {return const_iterator(const_cast<data_container*>(&end_container)); }
	  size_t			    size()		  const {return current_size;}
	  size_t 			    max_size()	const {return maximum_size;}
	  bool 			      empty()		  const {return current_size == 0;}
	  T&				      back()		  const {return end_container.previous->element;}
	  T&				      front()		  const {return root_container.next->element;}
  ...
  //--------------------------------------------------------------------------------
  template<class T>
  void list_s<T>::init(size_t new_size)
  {
	  //Assign sizes
	  ...
	  for (size_t i = 0; i < maximum_size-1; i++)
	  {
  ...
  //--------------------------------------------------------------------------------
  template<class T>
  list_s<T>::list_s(size_t new_size): data(NULL), next_free(NULL)
  {
  ...
  //--------------------------------------------------------------------------------
  template<class T>
  void list_s<T>::resize(size_t new_size)
  {
	  //Size must be at least 1
	  assert(new_size > 0);
  ...
  {
	  //Calculate new size
	  size_t new_size = ( (maximum_size * 3) >> 1) + 1;
